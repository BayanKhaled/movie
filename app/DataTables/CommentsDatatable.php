...
class CommentsDatatable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', '
                <a class="btn btn-default btn-outline-primary" href="comments/{{ $id }}/edit">Edit</a>
                <form action="comments/{{ $id }}" method="POST" onsubmit="return confirm(\'Are You sure?\');" style="display: inline-block;">
    ...
    protected function getColumns()
    {
        return [
            ...
            [
                ...
                'data' => 'status',
                'title' => 'Status',
                'render' => "'<span id=\"stauts\" class=\"right badge badge-success\"> ' + data + '</span>'",
            ],
            /*Column::make('status')->render("
                if (type === 'display') {
                    var country = '';
                    switch (data) {
                        case 'Argentina':
                ...
                return '<span class=\"right badge badge-danger\"> hii </span>';
                return '<span class=\"right badge badge-danger\"> ' + data + '</span>';

            ")*/
            /*Column::make('status')->render("return console.log(typeof data);
                return type === 'display' && data.length > 40 ? '<span title=\"'+data+'\">'+data+'...</span>' : data;
            "),*/
            Column::make('user.name')->title('User')->render("'<span class=\"right badge badge-success\"> ' + data + '</span>'"),
